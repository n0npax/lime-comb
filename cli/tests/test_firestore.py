...
def local_firestore(mocker, cred, document, collection_id, firestore_parent, document_id):
    ...
    a = cli.firestore.fetch.put_document(
        cred, firestore_parent, collection_id, document=document, document_id=document_id
    )
    print(a)
    yield
...
def test_get_doc(local_firestore,firestore_parent, collection_id, document_id):
    print(cli.firestore.fetch.get_document(cred, firestore_parent+f"/{collection_id}/{document_id}"))
